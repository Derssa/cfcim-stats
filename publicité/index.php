    ...
    <div class="app">
      ...
      <div class="body">
        ...
        <?php if($type==4){
          ...
          <a style="color: #0c4377" href="#">Publicité</a>
        </div>';
        }else{
          echo '<div class="subMenu">
          <a href="../entreprise">Entreprise</a>
        ...
        <div class="lwest">
          <form>
            <span
              >*liste des publicités qui ont étés consulté le
              jour
            </span>
          ...
          <?php
                ...
                if($type==2){
                  $sql = "SELECT * FROM advertisement WHERE jour=STR_TO_DATE('$jour', '%Y-%m-%d')";
                  $result = $conn->query($sql);
                }else if($type>2){
                  $sql = "SELECT * FROM advertisement WHERE jour=STR_TO_DATE('$jour', '%Y-%m-%d') AND internal_id='$mycfcim'";
                  $result = $conn->query($sql);
                }
          ?>
          <table id="customers">
    ...
    $(document).ready(function () {
      $('#date').on('change',function(){
        $.ajax({url: "date.php?date="+$('#date').val(), success: function(result){
          $('#customers').html("<tr><th>Visiteurs</th><th>Visites</th></tr>"+result)          
        }});
      })
    });
  </script>
