    ...
    <div class="app">
      ...
      <div class="body">
        ...
        <?php if($type==4){
          ...
          <a style="color: #0c4377" href="#">Documents</a>
        ...
        <div class="lwest">
          <form>
            <span
              >*liste des documents/plaquettes qui ont étés ouverts le jour
            </span>
          ...
          <?php
                ...
                if($type==2){
                  $sql = "SELECT * FROM document WHERE jour=STR_TO_DATE('$jour', '%Y-%m-%d')";
                  $result = $conn->query($sql);
                }else if($type>2){
                  $sql = "SELECT * FROM document WHERE jour=STR_TO_DATE('$jour', '%Y-%m-%d') AND internal_id='$mycfcim'";
                  $result = $conn->query($sql);
                } ?>
          <table id="customers">
            <tr>
    ...
    $(document).ready(function () {
      $('#date').on('change',function(){
        $.ajax({url: "date.php?date="+$('#date').val(), success: function(result){
          $('#customers').html("<tr><th>Noms D’E/ses</th><th>Noms Docs</th><th>Visiteurs</th></tr>"+result)          
        }});
      })
    });
  </script>
